...
|---|---|---|
| v1.0 | — | Первоначальное ТЗ: localStorage-синхронизация, формула `computePlanets` с базой 7, штрафы S2, S3, S4 |
| v1.1 | 2026-05-17 | Реализация всех эндпоинтов в `main.go`. Добавлен тег `//go:build ignore` в `galaxy_gen.go` для устранения конфликта `func main()`. Полная реализация реестра в `galaxy.html` и `registry.html` |
| v1.2 | 2026-05-17 | **SSE вместо localStorage.** В `main.go` добавлен SSE-хаб и эндпоинт `GET /registry/stream`. `registry.html` переведён на `EventSource`. Дельта-рендеринг: рождения через `appendChild`, смерти через `[data-field]`. Статистика пересчитывается на клиенте (дебаунс 1500ms) |
| v1.3 | 2026-05-17 | **Ревизия `computePlanets` (Option B).** S2: бонус +1 для средней зоны 2–5, штраф −1 только для S2=0. S3: убран штраф для S3=0–2 — красные карлики стабильные хозяева планет. S4: убран штраф за 0 астероидов; накапливаемые бонусы +1 при S4≥4 и ещё +1 при S4≥7. Цель: среднее число планет ≥ 6 (было ~2.6) |
| v1.4 | 2026-05-17 | **Ревизия температурной физики.** Туманности охлаждают звёзды: −3 T/масса (было +15). Астероиды нагревают: +1 T/масса (было +3). Формации нагревают как астероиды: +1 T/масса (было 0). Охлаждение красных карликов: −0.2/тик (было −0.14). Жёлтых карликов: −1.0/тик (было −0.67). Цель: более частое образование белых карликов |
| v1.5 | 2026-05-17 | **Белые карлики: трансформация и взрыв.** Красный/жёлтый карлик при остывании теперь трансформируется на месте в белый карлик (было: рассыпается через `disperseObject`). Белые карлики не рекласифицируются обратно при поглощении массы (`starTypeFromMass` пропускается). При достижении `WD_NOVA_MASS = 600`: взрыв — туманности + осколки + формация с `birth_event_type = 'wd_nova'` (S7 = 5, нейтрально). Увеличен размер галактики: 200×200 → 300×300 (GALAXY_R: 80 → 120). Ускорено охлаждение: красные карлики −0.5/тик, жёлтые −2.0/тик. Исправлен счётчик смертей НЗ: BH-коллизия переведена из `nsDiedCool` в `nsDiedCollision`; Mode 1 и Mode 2 star+star коллизии теперь корректно учитывают гибель НЗ |
| v1.6 | 2026-05-17 | **Замедление охлаждения под размер галактики 300×300.** Красные карлики: −0.3/тик (было −0.5). Жёлтые карлики: −1.5/тик (было −2.0). Цель: сохранить частоту образования белых карликов при увеличенном поле симуляции |
| v1.7 | 2026-05-17 | **Балансировка температурных скоростей.** Синий гигант: +1.0/тик (было +4.7) — менее взрывной нагрев. Нейтронная звезда: −0.2/тик (было −0.34) — живёт ~2450 тиков до DEATH_LIMIT вместо ~1440. Белый карлик: −0.1/тик (было −0.05) — вдвое быстрее остывает и финально рассыпается |
| v1.8 | 2026-05-18 | **Исправление коллизий и счётчиков НЗ.** Звезда всегда поглощает не-звезду (формацию/облако) вне зависимости от массы. НЗ и белый карлик исключены из `starTypeFromMass` — тип не меняется при наборе массы (НЗ > 601 больше не становится синим гигантом). Добавлен `nsDiedCollision` в CNT и UI. Все star+star коллизии (Mode 1 и Mode 2) корректно учитывают гибель НЗ |
| v1.9 | 2026-05-18 | **Соотношение ЧД:НЗ = 1:1.** `supernovae % 3` → `supernovae % 2`: теперь каждая вторая сверхновая синего гиганта даёт ЧД (было 1 ЧД на 3 события, ~33%). Выравнивает баланс: было ~3–6 ЧД vs ~40 НЗ |
