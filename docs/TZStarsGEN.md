...
| S2 | Радиус | Влияние |
|---|---|---|
| 0–1 | 0–24 у.е. | Центр галактики: −2 планеты |
| 2–5 | 24–72 у.е. | Норма |
| 6–9 | 72–120 у.е. | Периферия: −1 планета, +1 к вероятности кольца |
...
|---|---|---|---|
| 0–2 | 1–300 | Красный карлик | −1 планета, только ближняя зона |
| 3–5 | 301–600 | Жёлтый карлик | +1 планета, норма |
| 6–8 | 601–900 | Синий гигант | −2 планеты, мощный ветер |
...
| S4 | Масса | Влияние |
|---|---|---|
| 0–2 | 0–90 | −1 планета, меньше каменистых тел |
| 3–5 | 91–180 | Норма |
| 6–9 | 181–300 | +1 планета каменистого типа, +1 к кольцам |
...
**GEN-код: `808000614`**  
**ΔN:** −3 −2 −2 −1 −1 −1 −1 = **−11** → N = clamp(7−11, 0, 12) = **0 планет**  
**Кольца:** S1 ≥ 7 нет влияния на кольца, S4=0, S7=1 → **0 колец**
...
**GEN-код: `034883400`**  
**ΔN:** 0 +0 +1 +1 +1 +0 +1 = **+4** → N = clamp(11, 0, 12) = **11 планет**  
**Кольца:** S4=8 → +1 → **1 кольцо**
...
function computeGenCode(r) {
  ...
  const S7 = { inflow:0, star_collision:1, asteroid_collision:2,
               star_cloud:3, formation_merge:4 }[r.birth_event_type] ?? 5;
...
### 7.5 Запись в реестр (вызов из `checkFormationBirth`)
...
### Синхронизация с тиком галактики

Автообновление **не таймерное** — оно привязано к тику галактики напрямую.

**Механизм:** `galaxy.html` при каждом тике симуляции обновляет `localStorage['ouroboros_tick']` текущим номером тика. `registry.html` слушает событие `storage` и перезапрашивает реестр при каждом изменении тика.

```javascript
// galaxy.html — в конце каждого тика симуляции
localStorage.setItem('ouroboros_tick', String(tick));

// registry.html — слушатель
window.addEventListener('storage', e => {
  if (e.key === 'ouroboros_tick') loadRegistry();
});
```

Это гарантирует: таблица обновляется ровно раз за тик, без лишних запросов и без рассинхрона.

**Настраиваемый режим (дополнительно):** в шапке `registry.html` — переключатель «Sync: per tick / every N sec». При выборе «every N sec» таймер запускается только если вкладка `galaxy.html` закрыта и `localStorage` не обновляется (fallback).

```javascript
let fallbackTimer = null;
window.addEventListener('storage', e => {
  if (e.key !== 'ouroboros_tick') return;
  clearTimeout(fallbackTimer);
  loadRegistry();
  fallbackTimer = setTimeout(startFallback, 3000); // если тики остановились
});
function startFallback() {
  fallbackTimer = setInterval(loadRegistry, refreshInterval);
}
```

Новые строки подсвечиваются зелёным на 2 секунды. Смерти — красным миганием строки.
...
### main.go
- [ ] Эндпоинт `GET /registry` — читает и отдаёт `star_registry.json`
- [ ] Эндпоинт `GET /registry/stats` — считает и отдаёт статистику
- [ ] Эндпоинт `POST /registry/birth` — append записи
- [ ] Эндпоинт `POST /registry/death` — patch записи по `star_id`
- [ ] Эндпоинт `POST /registry/reset` — перезаписывает файл как `[]`

### galaxy.html
- [ ] Добавить `birth_event_type` на объект формации при создании
- [ ] Добавить `asteroid_absorbed` на объект формации (инкрементировать при поглощении)
- [ ] `localStorage.setItem('ouroboros_tick', tick)` в конце каждого тика
- [ ] Функция `scanNeighbors(o)` — один проход при рождении
- [ ] Функция `computeGenCode(r)` — вычисление кода
- [ ] Функции `computePlanets(...)` и `computeRings(...)`
- [ ] Вызов `registerBirth(...)` в `checkFormationBirth`
- [ ] Вызов `registerBirth(...)` для начальных звёзд при `applyGalaxy`
- [ ] Вызов `registerDeath(...)` в `doSupernova`, `coolDeath`, `heatDeath`, `handleCollision`
- [ ] Кнопка «REGISTRY» в UI → открывает `registry.html`
- [ ] `POST /registry/reset` при `⟳ GEN`

### registry.html
- [ ] Панель статистики из раздела 9.3 (запрос `GET /registry/stats`)
- [ ] Таблица с колонками из раздела 8
- [ ] Синхронизация через `localStorage` + fallback-таймер (раздел 8)
- [ ] Переключатель «per tick / every N sec» в шапке
- [ ] Фильтры по типу, статусу, тику, числу планет
- [ ] Подсветка новых записей (зелёный) и смертей (красный)
