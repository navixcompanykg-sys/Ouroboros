...
| Астероид | +1 | Нагревает |
| Формация | +1 | Нагревает как астероид |
...
- Возникает при сверхновой синего гиганта (чередование с ЧД)
- Рассыпается при mass ≥ 1001 у.е.
- Счётчики: `nsDiedCool / nsDiedHeat / nsDiedMass / nsDiedCollision`
...
```
radiated = ceil(BH_MASS_LOSS + mass × BH_LOSS_PCT)  // 10 + 10%
bh.mass -= radiated
```
...
### Выбор ЧД или НЗ при сверхновой
...
- Рассыпается при mass ≥ 1001 → правило 60/40, режим `decay`
...
| GRAVITY_TABLE K_BASE | **0.0001** | База таблицы гравитации |
...
| AST_TEMP_PER_MASS | **1** | Нагрев от астероида |
...
| ЧД:НЗ при сверхновой | **1:1** | `supernovae % 2` |
...
| CORE_NEB_HEAT_PER_MASS | **50 T/у.е.** | Нагрев звезды при поглощении |
...
collapse_limit: 1000, death_limit: 10, nebula_temp_per_mass: 3, nebula_cooling_per_mass: 3, asteroid_temp_per_mass: 1, wd_nova_mass: 600. Стартовые T и dT/тик — см. §7.2.

### Чёрные дыры (`black_holes`)

mass_loss_per_tick: 10, mass_loss_pct: 0.1, radiation_radius: 10, heat_max: 0.14.
...
bg_threshold: 20, jet_deficit_step: 2, nebula_mass: 5, start_temp: 400, heat_per_mass: 50, drain_every: 20, cooling_per_tick: 8.
